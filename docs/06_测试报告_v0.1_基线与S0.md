...
| 依赖安装（S0 进行中） | python-dotenv / pydantic / tenacity / dateparser / FastAPI / uvicorn / python-jose / passlib / APScheduler / pytest / httpx / Jinja2 / python-docx / docxtpl / openai / langgraph / langchain-openai | 后台安装 |
| 降级预案 | LangGraph 装不上 → 自研等价状态机；无 LLM Key → 规则式解析 + 模板化结论 | 已备案 |
